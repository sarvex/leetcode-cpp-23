...
class DistanceLimitedPathsExist {
  ...
  int father[10000];
  int rank[10000];
  std::unordered_set<int> changed;// the nodes whose father has been modified since last snapshot
public:
  ...
  void Union(int a, int b) {
    ...
    if (fa != fb) {
      if (rank[fa] < rank[fb]) {
        father[fa] = fb;
        rank[fb] = std::max(rank[fb], rank[fa] + 1);
        changed.insert(fa);
      } else {
        father[fb] = fa;
        rank[fa] = std::max(rank[fa], rank[fb] + 1);
        changed.insert(fb);
      }
    }
  }
  ...
  int findSnapRoot(int node, int limit) {
    auto iter = lower_bound(snaps[node].begin(), snaps[node].end(), std::make_pair(limit, 0));
    iter = prev(iter, 1);
    int f = iter->second;
    if (f == node)
      return f;
