...
public:
  ...
  std::vector<std::vector<int>> findAllParents(std::vector<std::vector<int>> &graph, int m, int c, int t)
  {
    ...
    if (t == 1)
    {
      for (int c_next : graph[c])
        if (c_next != 0)
          neighbors.push_back({m, c_next, 2});
    }
    else
    {
      for (int m_next : graph[m])
        neighbors.push_back({m_next, c, 1});
    }
    return neighbors;
  }

  bool allChildrenWin(std::vector<std::vector<int>> &graph, int m, int c, int t)
  {
    if (t == 1)
    {
      for (int m_next : graph[m])
