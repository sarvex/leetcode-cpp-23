...
public:
  std::vector<int> sortItems(int n, int m, std::vector<int> &group, std::vector<std::vector<int>> &beforeItems)
  {
    ...
    unordered_map<int, std::vector<int>> groupItemsOrdered;
    ...
    std::vector<int> groupOrdered = topologySort(groups, next, inDegree);
  ...
  std::vector<int> topologySort(unordered_set<int> &nodes, unordered_map<int, unordered_set<int>> &next, unordered_map<int, int> &inDegree)
  {
    queue<int> q;
    std::vector<int> ret;
    for (auto node : nodes)
