#include <queue>
#include <vector>

class Solution {
public:
  std::vector<int> topo(int k, std::vector<std::vector<int>> &conditions) {
    std::vector<int> indegree(k + 1);
    std::vector<std::vector<int>> next(k + 1);
    for (auto &x: conditions) {
      next[x[0]].push_back(x[1]);
      indegree[x[1]] += 1;
    }

    std::queue<int> q;
    for (int i = 1; i <= k; i++)
      if (indegree[i] == 0)
        q.push(i);

    std::vector<int> rets;
    while (!q.empty()) {
      int cur = q.front();
      q.pop();
      rets.push_back(cur);
      for (auto x: next[cur]) {
        indegree[x]--;
        if (indegree[x] == 0)
          q.push(x);
      }
    }

    if (rets.size() != k) return {};

    return rets;
  }

  std::vector<std::vector<int>> buildMatrix(int k, std::vector<std::vector<int>> &rowConditions, std::vector<std::vector<int>> &colConditions) {
    std::vector<int> row = topo(k, rowConditions);
    std::vector<int> col = topo(k, colConditions);

    if (row.empty() || col.empty()) return {};

    std::vector<std::pair<int, int>> pos(k + 1);
    for (int i = 0; i < k; i++)
      pos[row[i]].first = i;

    for (int i = 0; i < k; i++)
      pos[col[i]].second = i;

    std::vector<std::vector<int>> matrix(k, std::vector<int>(k));
    for (int i = 1; i <= k; i++)
      matrix[pos[i].first][pos[i].second] = i;

    return matrix;
  }
};
