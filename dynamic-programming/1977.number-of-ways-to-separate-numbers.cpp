...
public:
  bool larger(std::string &s, int a, int b, int len) {
    for (int i = 0; i < len; i++)
      if (s[a + i] != s[b + i])
        return s[a + i] > s[b + i];
    return false;
  ...
  int numberOfCombinations(std::string num) {
    ...
    std::vector<std::vector<int>> accum(n + 2, std::vector<int>(n + 2));
    std::vector<std::vector<int>> lcs(n + 2, std::vector<int>(n + 2));
