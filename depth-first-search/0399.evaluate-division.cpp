...
class Solution {
  std::unordered_map<std::string, std::vector<std::pair<std::string, double>>> graph;//存储图的数据结构
...
public:
  ...
  double DFS(std::string start, std::string end, unordered_set<std::string> visited) {
    ...
    {
      std::string mid = a.first;                       // 选取一个位置叫mid
