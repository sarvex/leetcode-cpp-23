...
class Solution {
  ...
  int n;
  std::unordered_set<long long> rets;
...
public:
  int countCombinations(std::vector<std::string> &pieces, std::vector<std::vector<int>> &positions) {
    ...
    for (int state = 0; state < (1 << (3 * n)); state++) {
      int flag = 1;
      std::vector<int> dirs(n);
      for (int k = 0; k < n; k++) {
        int d = (state >> (3 * k)) % 8;
        if (pieces[k] == "rook" && d > 3) {
          flag = 0;
          break;
        }
        if (pieces[k] == "bishop" && d < 4) {
          flag = 0;
          break;
        }
        dirs[k] = d;
  ...
  void dfs(std::vector<std::vector<int>> &positions, std::vector<int> &dirs, int state) {
    for (int subset = state; subset > 0; subset = (subset - 1) & state) {
      std::vector<std::vector<int>> pos2 = positions;
      int flag = 1;
      for (int i = 0; i < n; i++) {
        if ((subset >> i) & 1) {
          pos2[i][0] += dir[dirs[i]].first;
          pos2[i][1] += dir[dirs[i]].second;
          if (pos2[i][0] < 1 || pos2[i][0] > 8) {
            flag = 0;
            break;
          }
          if (pos2[i][1] < 1 || pos2[i][1] > 8) {
            flag = 0;
            break;
          }
        }
  ...
  bool duplicate(std::vector<std::vector<int>> &pos) {
    for (int i = 0; i < n; i++)
      for (int j = i + 1; j < n; j++)
        if (pos[i] == pos[j])
          return true;
  ...
  long long getHash(std::vector<std::vector<int>> &pos) {
    long long ret = 0;
    for (int k = 0; k < n; k++) {
      ret = ret * 10 + pos[k][0];
      ret = ret * 10 + pos[k][1];
