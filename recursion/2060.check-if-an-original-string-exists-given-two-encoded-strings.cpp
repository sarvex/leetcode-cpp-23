...
public:
  bool possiblyEquals(std::string s1, std::string s2) {
    std::vector<std::string> t1 = parse(s1);
    std::vector<std::string> t2 = parse(s2);
  ...
  bool dfs(std::vector<std::string> &t1, int i, int num1, std::vector<std::string> &t2, int j, int num2) {
    ...
    long long hash = i * 1e9 + num1 * 1e6 + j * 1e3 + num2;
    if (visited.find(hash) != visited.end()) return false;

    if (i < t1.size() && isdigit(t1[i][0])) {
      std::unordered_set<int> nums = getNum(t1[i]);
      for (int x: nums) {
      ...
      std::unordered_set<int> nums = getNum(t2[j]);
      for (int x: nums) {
  ...
  std::unordered_set<int> getNum(std::string t) {
    int d = stoi(t);
    if (t.size() == 1)
      return {d};
    else if (t.size() == 2) {
